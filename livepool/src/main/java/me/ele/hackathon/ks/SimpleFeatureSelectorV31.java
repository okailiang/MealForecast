 ...
 */
@Deprecated
public class SimpleFeatureSelectorV31 {
    private static final Logger log = LoggerFactory.getLogger(SimpleFeatureSelectorV31.class);

    public String FEATURE_SELECTOR_PREFIX = "simple_feature_selector_v31";
    ...
    public String CLICK_OUTPUT_File;
    public String BUY_OUTPUT_File;
    ...
    public SimpleFeatureSelectorV31(int clickTimes, int buyTimes) {
        this.clickTimes = clickTimes;
        this.buyTimes = buyTimes;

        FEATURE_SELECTOR_PREFIX = FEATURE_SELECTOR_PREFIX + "_" + this.clickTimes + "_" + this.buyTimes;

        CLICK_OUTPUT_File = FEATURE_DATA_DIR + FEATURE_SELECTOR_PREFIX + "/click_train.txt";
        BUY_OUTPUT_File = FEATURE_DATA_DIR + FEATURE_SELECTOR_PREFIX + "/buy_train.txt";
        TEST_OUTPUT_File = FEATURE_DATA_DIR + FEATURE_SELECTOR_PREFIX + "/test.txt";

    }

    public static DatasetFile buildDataset(int clickTimes, int buyTimes, boolean recreateFile) {
        SimpleFeatureSelectorV31 selector = null;
        try {
            selector = new SimpleFeatureSelectorV31(clickTimes, buyTimes);
            if (recreateFile) {
                ...
                selector.formatTrainDataset(INFO_SOURCE_FILE, ENV_SOURCE_FILE,
                        selector.CLICK_OUTPUT_File,
                        selector.BUY_OUTPUT_File);
        ...
        log.warn("=={}== 将rst_id 0／1向量化", selector.FEATURE_SELECTOR_PREFIX);
        log.warn("==构建{}选择的数据== 需确认已执行过SimpleFeatureSelector的main()将数据文件生成。", selector.FEATURE_SELECTOR_PREFIX);
        return new DatasetFile(selector.FEATURE_SELECTOR_PREFIX, selector.CLICK_OUTPUT_File, selector.BUY_OUTPUT_File, selector.TEST_OUTPUT_File, 11 + 70);
    }
