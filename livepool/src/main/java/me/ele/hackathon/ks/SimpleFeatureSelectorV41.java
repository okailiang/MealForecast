 ...
 * v41: 加入 最小起送价
 */
public class SimpleFeatureSelectorV41 {
    private static final Logger log = LoggerFactory.getLogger(SimpleFeatureSelectorV41.class);

    public String VERSION = "simple_feature_selector_v41";
    ...
    public SimpleFeatureSelectorV41(int clickTimes, int buyTimes) {
    ...
    public static DatasetFile buildDataset(int clickTimes, int buyTimes, boolean recreateFile) {
        SimpleFeatureSelectorV41 selector = null;
        try {
            selector = new SimpleFeatureSelectorV41(clickTimes, buyTimes);
            if (recreateFile) {
    ...
    private void formatTrainDataset(String info, String env,
                                    String clickExpand, String clickOrigin,
                                    String buyExpand, String buyOrigin) throws IOException {
        Map<String, AdList> adListMap = readAdList(env);

        List<Ad> expandAds = Lists.newArrayList();
        ...
        Collections.shuffle(expandAds, new Random(1234L));

        System.out.println("done. 扩容后size: " + expandAds.size() + " click, buy: " + clickCount + "," + buyCount + ", " +
                "click_rate: " + clickCount / expandAds.size() + ", buy_rate: " + buyCount / expandAds.size());

        outputClickFile(expandAds, clickExpand);
        outputClickFile(originAds, clickOrigin);
        outputBuyFile(expandAds, buyExpand);
        outputBuyFile(originAds, buyOrigin);
        log.warn("Format Train Dataset File ok, \nclickOutput: {}\n{}\n, buyOutput: {}\n{}\n", clickExpand, clickOrigin,
                buyExpand, buyOrigin);
    }
