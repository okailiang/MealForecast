 ...
 */
public class SimpleFeatureSelectorV43 {
    private static final Logger log = LoggerFactory.getLogger(SimpleFeatureSelectorV43.class);

    public String VERSION = "simple_feature_selector_v43";
    ...
    public SimpleFeatureSelectorV43() {
        this.clickTimes = 20;
        this.buyTimes = 120;
    ...
    public static DatasetFile buildDataset(boolean recreateFile) {
        SimpleFeatureSelectorV43 selector = null;
        try {
            selector = new SimpleFeatureSelectorV43();
            if (recreateFile) {
    ...
    private void formatTrainDataset(String info, String env,
                                    String clickExpandFile, String clickOriginFile,
                                    String buyExpandFile, String buyOriginFile) throws IOException {
        Map<String, AdList> adListMap = readAdList(env);

        List<Ad> expandClick = Lists.newArrayList();
        List<Ad> expandBuy = Lists.newArrayList();
        List<Ad> originAds = Lists.newArrayList();
        ...
        Collections.shuffle(expandClick, new Random(1234L));
        Collections.shuffle(expandBuy, new Random(1234L));

        log.warn("Selector [{}] done. 扩容后size: \n" +
                        "origin_size {}, click_expand {}, buy_expand {}, \n" +
                        "click_rate: {} , buy_rate: {}",
                VERSION, originAds.size(), expandClick.size(), expandBuy.size(),
                (double)clickCount / expandClick.size(),
                (double)buyCount / expandBuy.size());

        outputClickFile(expandClick, clickExpandFile);
        outputClickFile(originAds, clickOriginFile);
        outputBuyFile(expandBuy, buyExpandFile);
        outputBuyFile(originAds, buyOriginFile);
        log.warn("Format Train Dataset File ok, \nclickOutput: {}\n{}\n, buyOutput: {}\n{}\n", clickExpandFile, clickOriginFile,
                buyExpandFile, buyOriginFile);
    }
