...
/**
 * v41: 加入 最小起送价
 * v42: click 扩容1：1， buy扩容1：1
 * v43: 去除rest_id向量化，仅取id
 * v51: 去除index > 50的，以时间分隔train and test
 */
public class SimpleFeatureSelectorV51 {
    private static final Logger log = LoggerFactory.getLogger(SimpleFeatureSelectorV51.class);

    public String VERSION = "simple_feature_selector_v51";
    ...
    public String CLICK_EXPAND_OUTPUT_File;
    public String CLICK_ORIGIN_OUTPUT_File;
    public String BUY_EXPAND_OUTPUT_File;
    public String BUY_ORIGIN_OUTPUT_File;
    public String TEST_OUTPUT_File;
    ...
    public SimpleFeatureSelectorV51() {
        this.clickTimes = 20;
        this.buyTimes = 120;

        VERSION = VERSION + "_" + this.clickTimes + "_" + this.buyTimes;

        CLICK_EXPAND_OUTPUT_File = FEATURE_DATA_DIR + VERSION + "/click_expand.txt";
        CLICK_ORIGIN_OUTPUT_File = FEATURE_DATA_DIR + VERSION + "/click_origin.txt";
        BUY_EXPAND_OUTPUT_File = FEATURE_DATA_DIR + VERSION + "/buy_expand.txt";
        BUY_ORIGIN_OUTPUT_File = FEATURE_DATA_DIR + VERSION + "/buy_origin.txt";
        TEST_OUTPUT_File = FEATURE_DATA_DIR + VERSION + "/test.txt";
    }

    public SimpleFeatureSelectorV51(int click, int buy) {
        this.clickTimes = click;
        this.buyTimes = buy;
    ...
    public static DatasetFile buildDataset(boolean recreateFile) {
        SimpleFeatureSelectorV51 selector = null;
        try {
            selector = new SimpleFeatureSelectorV51();
            if (recreateFile) {
                log.warn("==buildDataset== Going to write dataset to disk.");
                restaurantMap = readRestaurant();

                selector.formatTrainDataset(INFO_SOURCE_FILE, ENV_SOURCE_FILE,
                        selector.CLICK_EXPAND_OUTPUT_File, selector.CLICK_ORIGIN_OUTPUT_File,
                        selector.BUY_EXPAND_OUTPUT_File, selector.BUY_ORIGIN_OUTPUT_File);
                selector.formatTestDataset(TEST_INFO_SOURCE_FILE,
                        TEST_ENV_SOURCE_FILE, selector.TEST_OUTPUT_File);
        ...
        log.warn("=={}== 将rst_id 0／1向量化", selector.VERSION);
        log.warn("==构建{}选择的数据== 需确认已执行过SimpleFeatureSelector的main()将数据文件生成。", selector.VERSION);
        return new DatasetFile(selector.VERSION, selector.CLICK_EXPAND_OUTPUT_File, selector.CLICK_ORIGIN_OUTPUT_File,
                selector.BUY_EXPAND_OUTPUT_File, selector.BUY_ORIGIN_OUTPUT_File,
                selector.TEST_OUTPUT_File,
                13);
    }

    public static DatasetFile buildDataset(boolean recreateFile, int click, int buy) {
        SimpleFeatureSelectorV51 selector = null;
        try {
            selector = new SimpleFeatureSelectorV51(click, buy);
            if (recreateFile) {
                log.warn("==buildDataset== Going to write dataset to disk.");
                restaurantMap = readRestaurant();
    ...
    private static Map<String, Restaurant> readRestaurant() throws IOException {
        ...
        while ((line = in.readLine()) != null) {
            ...
            double min_deliver_amount = Double.parseDouble(list.get(13));
            ...
            Restaurant restaurant = new Restaurant(x, y, isPremium, hasImage, isTimeEnsure, isKa,
                    isTimeEnsureDisCount, isElemeDeliver, isPromotionInfo, min_deliver_amount);
    ...
    private void formatTrainDataset(String info, String env,
                                    String clickTrainFile, String clickTestFile,
                                    String buyTrainFile, String buyTestFile) throws IOException {
        Map<String, AdList> adListMap = readAdList(env);

        List<Ad> clickTrain = Lists.newArrayList();
        List<Ad> buyTrain = Lists.newArrayList();
        List<Ad> test = Lists.newArrayList();
        ...
        while ((line = in.readLine()) != null) {
            ...
            if (adList == null) {
                ...
                if (index <= 50) {
                    ...
                    if (ad.getDay_no() > 136) { // last two week
                        test.add(ad);
                    } else {
                        if (isClick == 1) {
        ...
        Collections.shuffle(clickTrain, new Random(1234L));
        Collections.shuffle(buyTrain, new Random(1234L));

        log.warn("Selector [{}] done. 扩容后size: \n" +
                        "test_size {}, click_expand {}, buy_expand {}, \n" +
                        "click_rate: {} , buy_rate: {}",
                VERSION, test.size(), clickTrain.size(), buyTrain.size(),
                (double) clickCount / clickTrain.size(),
                (double) buyCount / buyTrain.size());

        outputClickFile(clickTrain, clickTrainFile);
        outputClickFile(test, clickTestFile);
        outputBuyFile(buyTrain, buyTrainFile);
        outputBuyFile(test, buyTestFile);
        log.warn("Format Train Dataset File ok, \n\tclickOutput: {}\t{}\n\tbuyOutput: {}\t{}\n", clickTrainFile,
                clickTestFile,
                buyTrainFile, buyTestFile);
    }
    ...
    private static void outputClickFile(List<Ad> ads, String clickOutput) throws IOException {
        ...
        try (PrintWriter pw = new PrintWriter(Files.newBufferedWriter(
            ...
            ads.stream().forEach(ad -> {
                ...
                pw.append("12:").append(String.valueOf(ad.getMin_deliver_amount())).append(SEPARATOR);


                pw.append("13:").append(String.valueOf(ad.getRst_id())).append(SEPARATOR);
    ...
    public static class Ad {
        ...
        public Ad(int index, int isClick, int isBuy, AdList adList, Restaurant rst) {
            ...
            this.min_deliver_amount = rst.getMin_deliver_amount();
            this.day_no = adList.getDay_no();
        }
        ...
        public double getMin_deliver_amount() {
    ...
    public static class AdList {
        ...
        public AdList(String id, int isSelect, int isNew, double x, double y, int day_no) {
            this.id = id;
    ...
    private static class Restaurant {
        ...
        public Restaurant(double x, double y, int is_premium, int has_image, int is_time_ensure, int is_ka,
                          int is_time_ensure_discount, int is_eleme_deliver, int is_promotion_info, double min_deliver_amount) {
            this.x = x;
            this.y = y;
